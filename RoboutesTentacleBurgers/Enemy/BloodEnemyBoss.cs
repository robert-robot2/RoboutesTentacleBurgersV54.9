...
public class BloodEnemyBoss
{
    ...
    //   public void StopMovement(string key) => StopMovementWarrior(key);
       ...
       public int EnemyY => SkeletonY;
        ...
        {
            levelReg.AddXp("SkelBoss", multiplier);
        }
        ...
        public int SkeletonRenderWidth { get; set; } = 256;   // how big you want it on screen
        public int SkeletonRenderHeight { get; set; } = 256;

        public string SkeletonRenderWidthPx => $"{SkeletonRenderWidth}px";
        public string SkeletonRenderHeightPx => $"{SkeletonRenderHeight}px";
        ...
        public int SkeletonMaxHP { get; set; } = 1000;
        public int SkeletonStatPoints { get; set; } = 0;
        public int SkeletonHitPoints { get; set; } = 1000;

        private int skeletonDamageAmount = 10;
        ...
        public const int SkeletonFrameWidth = 256;
        public const int SkeletonFrameHeight = 256;
        ...
        public string SkeletonSpriteStyle =>
            ...
            $"width:{SkeletonRenderWidth}px; height:{SkeletonRenderHeight}px; " +
            $"background-image:url('{SkeletonSpriteSheet}'); " +
            $"background-position:-{skeletonFrame * SkeletonFrameWidth}px 0px; " +
            $"background-size:{SkeletonRenderWidth * animationFrameCounts[currentSkeletonAnimation]}px {SkeletonRenderHeight}px; " +
            $"background-repeat:no-repeat; background-color:transparent; " +
            $"opacity:0.95; z-index:{SkeletonZIndex};";
        ...
        private readonly Dictionary<SkeletonAnimationState, int> animationFrameCounts = new()
{
 ...
 { SkeletonAnimationState.WalkDown, 8}
        ...
        public void TickSkeletonMovement(IBloodiCharacter active)
        {
            ...
            if (!skeletonIsTired && rng.NextDouble() < 0.02) // ~2% chance per tick
            {
                skeletonIsTired = true;
                skeletonTiredUntil = DateTime.Now.AddMilliseconds(rng.Next(1000, 2000)); // tired for 1–2s
            }
