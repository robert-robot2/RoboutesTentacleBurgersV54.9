...
using WeatherParticle = RoboutesTentacleBurgers.SpectralXComponent.SpectralXWeather.WeatherParticleType2;
...
namespace RoboutesTentacleBurgers.SpectralXComponent
{
    public class SpectralXEngine
    {
        ...
        public CameraMode ActiveCameraMode { get; set; } = CameraMode.WebpageView;
        public void SetCameraMode(CameraMode mode)
        {
            ActiveCameraMode = mode;
            if (mode == CameraMode.WebpageView)
            {
                Camera.Rotation = new Vec3(0f, 0f, 0f);
                Camera.Position = new Vec3(Camera.RailX, Camera.RailY, Camera.Position.Z);
            }
        }
        ...
        private readonly SpectralXGLXx SpectralXGLX;
    ...
    { "PinkPointGizmo",       17 },
      ...
      0.1f, 2000f);
        ...
        public void Init()
        {
            ...
            Weather.Init(Scene, MeshLibrary, new Dictionary<WeatherParticleType2, ParticleVolume>
{
...
{ WeatherParticleType2.Rain, new ParticleVolume(
    -18f, 18f,   // X — covers full text width
    -2f,  2f,    // Y — thin slab just in front of mesh
    8f,   13f    // Z — above text down through it
)},
    { WeatherParticleType2.Snow,      new ParticleVolume(-15f, 15f, -15f, 15f,  -50f, 20f) },
    { WeatherParticleType2.Cloud,     new ParticleVolume(-15f, 15f, -15f, 15f,  -50f, 20f) },
    { WeatherParticleType2.Lightning, new ParticleVolume(-15f, 15f, -15f, 15f,  -50f, 20f) },
          ...
          range: 15f);
           ...
           lightGizmo.EmissiveIntensity = 0.8f;
            ...
            if (cube != null)
            {
                cube.Position = new Vector3(1, 4, 2);
                cube.Size = new Vector3(5f, 5f, 5f);
                cube.Color = new Vector4(0f, 0f, 1f, 0.7f);
                Scene.AddMesh(cube);
            }

            var cube2 = MeshLibrary.GetMesh("PrimCube");
            if (cube2 != null)
            {
                cube2.Position = new Vector3(1, 4, 2);
                cube2.Size = new Vector3(0.5f, 0.5f, 0.5f);
                cube2.Color = new Vector4(0f, 0f, 1f, 0.7f);
                Scene.AddMesh(cube2);
            }
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                {
                    // 30% chance to skip = creates paths
                    if (rand.NextDouble() < 0.3)
                        continue;

                    var cube3 = MeshLibrary.GetMesh("PrimCube");

                    if (cube3 != null)
                    {
                        cube3.Position = new Vector3(x * spacing, 0, z * spacing);
                        cube3.Size = new Vector3(0.5f, 0.5f, 0.5f);

                        // vary color slightly for style
                        cube3.Color = new Vector4(
                            0f,
        ...
        */
            ...
            /*
            // ── HOME Title ───────────────────────────────────────────────────────────
            var aboutText = AddText("Home",
                position: new Vector3(-2f, 0f, 11f),
                fontSize: 2f,
                fontKey: "Diablo",
                color: new Vector4(0.2f, 0.8f, 1f, 0.35f),
                align: TextAlignment.Center);
            aboutText.Rotation = new Vector3(-MathF.PI / 2f, 0f, 0f);
            aboutText.GlowRadius = 0.02f;
            aboutText.GlowStrength = 0.3f;
            aboutText.GlowColor = new Vector4(0.2f, 0.8f, 1f, 0.5f);
            aboutText.ShadowBlur = 0.04f;
            aboutText.ShadowColor = new Vector4(0.2f, 0.8f, 1f, 0.3f);
            aboutText.GlowPulseSpeed = 1.0f;
            aboutText.GlowPulseMin = 0.01f;
            aboutText.GlowPulseMax = 0.04f;
            aboutText.EmissiveIntensity = 0.0f;
            */

            var roboute = MeshLibrary.GetMesh("Roboute");
            if (roboute != null)
            {
                Scene.AddMesh(roboute);
                roboute.Position = new Vector3(-16, 0, 11);
                roboute.Size = new Vector3(2f, 2f, 2f);
                roboute.Color = new Vector4(0f, 1f, 0f, 1f);
                roboute.Rotation += new Vector3(MathF.PI / 2f, 0f, 0f);    // X +90

            }
            var ctaText3 = AddText("The FrostWolf Clans Enemies Are Evil not innocent Children",
               position: new Vector3(-3f, 0f,10f),
               fontSize: 0.7f,
               fontKey: "Diablo",
               color: new Vector4(1f, 0.3f, 0.3f, 1f),
               align: TextAlignment.Center);
            ctaText3.Rotation = new Vector3(-MathF.PI / 2f, 0f, 0f);
            ctaText3.GlowRadius = 0.02f;
            ctaText3.GlowStrength = 0.5f;
            ctaText3.GlowColor = new Vector4(1f, 0.3f, 0.3f, 0.8f);
            ctaText3.ShadowBlur = 0.04f;
            ctaText3.ShadowColor = new Vector4(1f, 0f, 0f, 0.4f);
            ctaText3.GlowPulseSpeed = 2.0f;
            ctaText3.GlowPulseMin = 0.01f;
            ctaText3.GlowPulseMax = 0.06f;
            ...
            welcomeText.GlowRadius = 0.00f;
            welcomeText.GlowStrength = 0.5f;
            welcomeText.EmissiveIntensity = 0.5f;
            welcomeText.ShadowBlur = 0.3f;
            welcomeText.ShadowColor = new Vector4(1f, 0.5f, 0f, 0.8f);
            welcomeText.GlowColor = new Vector4(0f, 0.8f, 1f, 1f);
            welcomeText.GlowPulseSpeed = 1.5f;
            welcomeText.GlowPulseMin = 0.02f;
            welcomeText.GlowPulseMax = 0.25f;

            // ── Nightmare Tagline ────────────────────────────────────────────────────
            var projects2Text = AddText("Where your nightmares are dreams and your dreams are nightmares!",
                position: new Vector3(-3f, 0f, 6f),
                fontSize: 0.5f,
                fontKey: "Diablo",
                color: new Vector4(1f, 0.2f, 0.2f, 1f),
                align: TextAlignment.Center);
            projects2Text.Rotation = new Vector3(-MathF.PI / 2f, 0f, 0f);
            projects2Text.GlowRadius = 0.02f;
            projects2Text.GlowStrength = 0.5f;
            ...
            personalityText.GlowStrength = 0.5f;

            // ── Tech Stack ───────────────────────────────────────────────────────────
            var techText = AddText("C# · Blazor · WebGL · JavaScript · HTML · C/C++ · CSS · FBX Pipelines · Real-Time Rendering",
                position: new Vector3(-3f, 0f, -1.5f),
                fontSize: 0.7f,
                fontKey: "Diablo",
                color: new Vector4(1f, 0.2f, 0.2f, 1f),
            ...
            techText.GlowStrength = 0.5f;

            // ── Projects Callout ─────────────────────────────────────────────────────
            var projectsCallout = AddText("SpectralX WebGL Engine · BloodWyrm 2D Framework · Death Cube 3D Prototype",
                position: new Vector3(-3f, 0f, -3f),
                fontSize: 0.7f,
                    fontKey: "Diablo",
                color: new Vector4(1f, 0.2f, 0.2f, 1f),
            ...
            projectsCallout.GlowStrength = 0.5f;

            // ── CTA Line ─────────────────────────────────────────────────────────────
            var ctaText2 = AddText("Based in Warrington PA · Open to Full-Time Roles · Create Humans Dont Destroy!",
                position: new Vector3(-3f, 0f, -4.5f),
                fontSize: 0.7f,
                fontKey: "Diablo",
                color: new Vector4(1f, 0.3f, 0.3f, 1f),
                align: TextAlignment.Center);
            ctaText2.Rotation = new Vector3(-MathF.PI / 2f, 0f, 0f);
            ctaText2.GlowRadius = 0.02f;
            ctaText2.GlowStrength = 0.5f;
            ctaText2.GlowColor = new Vector4(1f, 0.3f, 0.3f, 0.8f);
            ctaText2.ShadowBlur = 0.04f;
            ctaText2.ShadowColor = new Vector4(1f, 0f, 0f, 0.4f);
            ctaText2.GlowPulseSpeed = 2.0f;
            ctaText2.GlowPulseMin = 0.01f;
            ctaText2.GlowPulseMax = 0.06f;
            ...
            var projectsText = AddText("PROJECTS",
                position: new Vector3(-3f, 0f, -22f),
            ...
            var sysText1 = AddText("Desktop and Laptop PC only for Games. Mobile for page view.",
                position: new Vector3(-3f, 0f, -26.2f),
                fontSize: 0.3f, fontKey: "Diablo",
            ...
            var sysText2 = AddText("CPU: Pentium I minimum — tick logic under 16ms for 60 FPS.",
                position: new Vector3(-3f, 0f, -27.0f),
                fontSize: 0.3f, fontKey: "Diablo",
                color: new Vector4(0.4f, 0.8f, 1f, 0.9f),
                align: TextAlignment.Center);
            ...
            var sysText3 = AddText("GPU: Nvidia Riva TNT, Intel onboard, or any WebGPU capable device.",
                position: new Vector3(-3f, 0f, -27.8f),
                fontSize: 0.3f, fontKey: "Diablo",
                color: new Vector4(0.4f, 0.8f, 1f, 0.9f),
                align: TextAlignment.Center);
            ...
            var sysText4 = AddText("Browser: Latest Chrome, Edge, or Firefox with WebAssembly and WebGPU.",
                position: new Vector3(-3f, 0f, -28.6f),
                fontSize: 0.3f, fontKey: "Diablo",
                color: new Vector4(0.4f, 0.8f, 1f, 0.9f),
                align: TextAlignment.Center);
            ...
            var sysText5 = AddText("OS: Windows 3.1, Linux, or any OS supporting browser capabilities. DOS loads.",
                position: new Vector3(-3f, 0f, -29.4f),
                fontSize: 0.3f, fontKey: "Diablo",
                color: new Vector4(0.4f, 0.8f, 1f, 0.9f),
                align: TextAlignment.Center);
            ...
            var sysText6 = AddText("Network: Interweeb or Waifu required.",
                position: new Vector3(-3f, 0f, -30.2f),
                fontSize: 0.3f, fontKey: "Diablo",
                color: new Vector4(0.4f, 0.8f, 1f, 0.9f),
                align: TextAlignment.Center);
            ...
            var sysText7 = AddText("Civilization Requirement: Electricity. Maybe chair. Snack recommended.",
                position: new Vector3(-3f, 0f, -31.0f),
                fontSize: 0.3f, fontKey: "Diablo",
                color: new Vector4(0.4f, 0.8f, 1f, 0.9f),
                align: TextAlignment.Center);
            ...
            var sysText8 = AddText("Performance Law: Objects update at sovereign cadence. 60 FPS. Scalable to 120.",
                position: new Vector3(-3f, 0f, -31.8f),
                fontSize: 0.3f, fontKey: "Diablo",
                color: new Vector4(0.4f, 0.8f, 1f, 0.9f),
                align: TextAlignment.Center);
            ...
            var sysText9 = AddText("Cache Warning: Clear web cache when new updates are released.",
                position: new Vector3(-3f, 0f, -32.6f),
                fontSize: 0.3f, fontKey: "Diablo",
            ...
            var sysText10 = AddText("If server is inaccessible during patches please check back later.",
                position: new Vector3(-3f, 0f, -33.4f),
                fontSize: 0.3f, fontKey: "Diablo",
                color: new Vector4(1f, 0.9f, 0.2f, 0.9f),
                align: TextAlignment.Center);
            ...
            var techNote = AddText("C# OOP · SpectralGL WebGL2 GPU Backend · CLR / WASM NetCore 10",
                position: new Vector3(-3f, 0f, -34.2f),
                fontSize: 0.3f, fontKey: "Diablo",
                color: new Vector4(0.4f, 0.8f, 1f, 0.9f),
                align: TextAlignment.Center);
         ...
         skySphere.MaterialTextures.Add("/iAssets/Galaxy0001.png");
         Scene.AddMesh(skySphere);
        ...
        private void InitScene2()
        {
            ...
            Weather = new SpectralXWeatherClass();
            Weather.Init(Scene2, MeshLibrary, new Dictionary<WeatherParticleType2, ParticleVolume>
{
    { WeatherParticleType2.Rain,      new ParticleVolume(-64f, 64f, -64f, 64f,  0f, 30f) },
    { WeatherParticleType2.Snow,      new ParticleVolume(-64f, 64f, -64f, 64f,  0f, 30f) },
    { WeatherParticleType2.Cloud,     new ParticleVolume(-64f, 64f, -64f, 64f, 30f, 70f) },
    { WeatherParticleType2.Lightning, new ParticleVolume(-64f, 64f, -64f, 64f, 30f, 70f) },
});
       ...
       SpectralXPropScatter.DeriveFootprintRadius(house17));
            ...
            */
            ...
            var skySphere = CreateGizmoFrom("SkySphere", "SmoothSphere");
            skySphere.Position = new Vector3(Camera.Position.X, Camera.Position.Y, Camera.Position.Z);
            skySphere.Size = new Vector3(40f, 40f, 40f);
            skySphere.Color = new Vector4(1f, 1f, 1f, 1f);
            skySphere.IsEmissive = true;
            skySphere.EmissiveIntensity = 1.0f;
            skySphere.MaterialTextures.Add("/iAssets/SkyDay004SL.png");
    ...
    TextAlignment align = TextAlignment.Left)
        ...
        public WebGLFrameData TickAndGetFrame()
        {
            ...
            SyncSkySphere();      
            TickSun();
            if (ActiveCameraMode == CameraMode.WebpageView)
            {
        ...
        {
            if (ActiveScene != 1 || _sunLight == null) return;
        ...
        private void TickAnimations()
        {
            ...
            _lastFrameDelta = delta;  // ADD THIS
            _lastTickTime = now;
        ...
        {
            // remove guard from scene 2 to have weather in all scenes.|| ActiveScene != 2
            ...
            var particleTextures = new[]
 {
    ...
    ("ParticleGeo_/iAssets/LBolt002.png",        "/iAssets/LBolt002.png"),
         ...
         1f, 500f);
                    ...
                    {
                        // Point light — aim at center of Scene 1 cube grid
                        Vec3 lookTarget = ActiveScene == 1
                            ? new Vec3(5f, 5f, 0f)   // center of cube grid
                            : new Vec3(0f, 8f, 2f);  // Scene 2 village center

                        Mat4 lView = Mat4.CreateLookAt(lPos, lookTarget, new Vec3(0, 0, 1));
            ...
            // remove scene 2 guard--->ActiveScene == 2 &&
    ...
    0.1f, 2000f),
                ...
                SkyDayTexUrl = ActiveScene == 1 ? "/iAssets/SkyDay004SL.png" : null,
                SkyNightTexUrl = ActiveScene == 1 ? "/iAssets/Galaxy0001.png" : null,
        ...
        private List<WebGLTextData> BuildTextFrameData(Mat4 vp)
        {
          ...
          //  Console.WriteLine($"[TextSystem] BuildTextFrameData — checking {activeScene.Meshes.Count} meshes");
            foreach (var mesh in activeScene.Meshes)
            {
