...
namespace RoboutesTentacleBurgers.SpectralXComponent.SpectralXWeather
{
    ...
    public class SpectralXParticleManager
    {
        ...
        private Dictionary<WeatherParticleType2, ParticleVolume> _volumes = new();
            ...
            Dictionary<WeatherParticleType2, ParticleVolume> volumes)
        {
            ...
            AllocatePool(_rainPool, RainPoolSize, WeatherParticleType2.Rain,
                "/iAssets/RainDrop01.png", 0.15f, 0.5f);

            AllocatePool(_snowPool, SnowPoolSize, WeatherParticleType2.Snow,
                "/iAssets/SnowFlake01.png", 0.3f, 0.3f);

            AllocatePool(_cloudPool, CloudPoolSize, WeatherParticleType2.Cloud,
                "/iAssets/GOkuCloud001.png", 6.0f, 4.0f);

            AllocatePool(_lightningPool, LightningPoolSize, WeatherParticleType2.Lightning,
        ...
        private List<SpectralXParticle>? GetPool(WeatherParticleType2 type) => type switch
        {
            WeatherParticleType2.Rain => _rainPool,
            WeatherParticleType2.Snow => _snowPool,
            WeatherParticleType2.Cloud => _cloudPool,
            WeatherParticleType2.Lightning => _lightningPool,
            _ => null
        };

        public IEnumerable<SpectralXParticle> GetPoolSlots(WeatherParticleType2 type) => type switch
        {
            WeatherParticleType2.Rain => _rainPool,
            WeatherParticleType2.Snow => _snowPool,
            WeatherParticleType2.Cloud => _cloudPool,
