...
using RoboutesTentacleBurgers.SpectralXComponent.SpectralXDebug;

namespace RoboutesTentacleBurgers.SpectralXComponent
{
    public class SpectralXInput
    {
        public readonly SpectralXGLXx SpectralXGLX;
        ...
        public void HandleScrollbarMouseDown(MouseEventArgs e)
        {
            ...
            // Check if click is in right 4% of viewport
            if (e.OffsetX > Viewport.ViewportWidth * 0.96f)
            {
        ...
        public void HandleScrollbarMouseMove(MouseEventArgs e)
        {
            ...
            float t = 1.0f - ((float)e.OffsetY / Viewport.ViewportHeight);
            float range = Camera.MaxZ - Camera.MinZ;
            Camera.TargetZ = Math.Clamp(Camera.MinZ + t * range, Camera.MinZ, Camera.MaxZ);
        }
        ...
        public SpectralXInput(SpectralXGLXx spectralX, SpectralXViewport viewport, SpectralXCamera camera,IJSRuntime js)
        {
        ...
        {
            if (_engine?.ActiveCameraMode == SpectralXEngine.CameraMode.WebpageView)
        ...
        {
            if (isRightMouseDown && _engine?.ActiveCameraMode == SpectralXEngine.CameraMode.FreeCam)
            {
