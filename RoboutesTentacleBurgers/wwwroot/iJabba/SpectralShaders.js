...
window._SpectralShaders = {


    // PCF Shadows

    fsSourcePCF: `#version 300 es
    ...
    // Self-shadow gate — fragments very close to the light source
    // are part of the light fixture itself (bulb, socket, gizmo)
    // Skip shadow sampling entirely and return fully lit
    ...
    fsSourceSpectralXSV1: `#version 300 es
                precision mediump float;
    ...
    vec3 areaRight = normalize(cross(uLightDir[i], upRef));
        ...
        totalDiffuse  += shadowContrib * diff * uLightColor[i] *
            uLightIntensity[i] * attenuation;
        totalSpecular += shadowContrib * spec * uLightColor[i] *
            uLightIntensity[i] * attenuation * 0.3;
    }
    ...
    // SpectralX RPD Shadow V2 — Rotated Poisson Disk
    // Per-pixel world-position hash rotates a fixed 16-point mathematically
    // optimized disk — breaks staircase by making every pixel sample uniquely
    fsSourceSpectralXSV2: `#version 300 es
            precision mediump float;
    ...
    // SpectralX IGN Shadow V3 — Interleaved Gradient Noise — placeholder
    // Will be implemented with spatiotemporal noise + TAA accumulation
    // what it needs to be inside the object:


    fsSourceSpectralXSV3: null, // placeholder, set after object closes
...
window._SpectralShaders.fsSourceSpectralXSV3 = window._SpectralShaders.fsSourceSpectralXSV2;
